...
export const createMainScreenPage = (): MainScreenView => {
  ...
  subtitle.textContent =
    "Face the camera and hold still. Only one foreground face can be verified at a time.";
  ...
  const setDetectingMode = (): void => {
    ...
    setStage(
      "detecting",
      "Detecting face",
      "Keep your face centered and close to the camera guide.",
    );
    setProgressTarget(12);
    camera.setFaceOverlay(null);
  };

  const showRecognized = (
    message: string,
    name: string | null,
    similarity: number,
  ): void => {
    ...
    resultTitle.textContent = "Face recognised";
    resultCopy.textContent = message || (name
      ? `Welcome back, ${name}.`
  ...
  const showUnrecognized = (message: string): void => {
    ...
    resultMeta.textContent =
      "You can set up FaceID here or continue with the mobile app.";
    resultActions.style.display = "grid";
    ...
    setStage(
      "unrecognized",
      "Verification failed",
      "Choose how you want to set up FaceID.",
    );
    setProgressTarget(100);
    loopPaused = true;
  };
  ...
  const showError = (message: string): void => {
    ...
    resultCopy.textContent = message;
    resultMeta.textContent =
      "Check camera alignment or backend connectivity, then retry.";
    resultActions.style.display = "grid";
    desktopSetupButton.style.display = "none";
    mobileSetupButton.style.display = "none";
  ...
  const startDesktopEnrollment = (): void => {
    ...
    setStage(
      "desktop-setup",
      "Desktop FaceID setup",
      "Capture each pose to start enrollment.",
    );
  ...
  const toggleMobilePanel = (): void => {
    ...
    mobileCopy.textContent =
      "Scan this QR with your phone to continue FaceID setup in the mobile app.";
  ...
  const runVerification = async (
    ...
    // Get the JPEG as a Blob and create a File
    const jpegData = camera.captureFrameJpeg(640, 0.9);
    if (!jpegData) {
      return;
    }
    ...
    function base64ToFile(base64: string, filename: string): File {
      ...
      if (!match) {
        throw new Error("Could not parse MIME type from base64 string");
      }
      const mime = match[1];
    ...
    const file = base64ToFile(jpegData, "face.jpg");

    verifying = true;
    setStage(
      "capturing",
      "Capturing frame",
      "Face captured. Preparing verification request.",
    );
    ...
    try {
      setStage(
        "uploading",
        "Sending to API",
        "Uploading captured face to verification service.",
      );
      ...
      setStage(
        "verifying",
        "Verifying response",
        "Evaluating recognition response and policy checks.",
      );
      setProgressTarget(85);

      const employeeName =
        response.employee?.name && typeof response.employee.name === "string"
    ...
    try {
      ...
      if (response.reasonCode === "multiple-faces") {
        setStage(
          ...
          "Only one foreground face can be in front of the camera.",
        );
      ...
      if (response.reasonCode === "face-out-of-zone") {
        setStage(
          "detecting",
          "Move closer",
          "Center your face inside the guide and stay still.",
        );
      ...
      if (!response.detected) {
        setStage(
          "detecting",
          "Face not found",
          "Align your face to start verification.",
        );
        setProgressTarget(8);
        return;
      }
      ...
      if (response.hasSingleForegroundFace) {
        setStage(
          "detecting",
          "Face locked",
          "Stable face found. Starting verification sequence.",
        );
  ...
  desktopCaptureButton.addEventListener("click", () => {
    ...
    if (!pose) {
      desktopProgress.textContent =
        "Enrollment samples complete. You can now submit these images to FaceID enrollment API.";
      setProgressTarget(100);
      return;
    }
    ...
    if (!image) {
      desktopProgress.textContent =
        "Camera frame not ready. Hold still and retry capture.";
      return;
    }
    ...
    desktopProgress.textContent =
      "5 captures complete. Submit these samples to your FaceID enrollment endpoint.";
    setProgressTarget(100);
  });
  ...
  return {
    ...
    onShow: async () => {
      ...
      setStage(
        "warming",
        "Initializing camera",
        "Starting webcam and loading face detector.",
      );
      setProgressTarget(4);
