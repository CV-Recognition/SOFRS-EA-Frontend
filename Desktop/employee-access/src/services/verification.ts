...
const DEFAULT_API_BASE_URL =
  import.meta.env.VITE_DEFAULT_API_BASE_URL || "https://api.example.com";
...
const getApiBaseUrl = (): string =>
  getConfiguredValue(import.meta.env.VITE_API_BASE_URL) ?? DEFAULT_API_BASE_URL;

const getVerifyEndpoint = (): string => {
  const explicitEndpoint = getConfiguredValue(
...
export const checkApiHealth = async (): Promise<boolean> => {
  try {
    const response = await fetch(
      new URL(HEALTH_PATH, getApiBaseUrl()).toString(),
      {
        method: "GET",
        headers: {
          "X-API-Key": API_KEY,
